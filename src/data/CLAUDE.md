...
**Slots de "Materia N de Área de Concentración"** (`AREA_CONCENTRACION_SLOT_RE`): mismo mecanismo de captura que optativas (mismos dos puntos de detección en `parse_pdf`, mismo patrón de preservar `current_semester` en el momento de la detección) — representan una materia obligatoria de esa etapa del plan, no una optativa libre. A diferencia de optativas, se preserva el número que ya trae el PDF (`AREA-{n}`, `nombre: "Materia {n} de Área de Concentración"`) en vez de renumerar — es estable y sin ambigüedad, porque cada slot de área sí trae un número propio en el PDF (las optativas no).
