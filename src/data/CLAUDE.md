...
| `pending_prereq_groups` | Filas de continuación de prerreqs acumuladas desde el último reinicio, cada una como `(top, words)` — overflow de la columna prereq, de atribución ambigua hasta que aparece la siguiente materia (ver "Desborde de prerreqs hacia abajo" abajo) |
| `last_saved_course_id` / `last_saved_top` | Clave y `top` de la última materia guardada **directo** (todo en una fila), mientras sigue "elegible" para recibir desborde de sus propios prerreqs hacia la línea de abajo |
...
**Desborde de prerreqs hacia abajo** (confirmado en `DAC-A`: `COM-22108`/`ACT-13307`, issues #4/#5 — y en el catálogo compartido de Estudios Generales/Finanzas que reaparece en decenas de PDFs, ej. `EGN-17123`, `ADM-15507`): normalmente, cuando la lista de prerreqs de una materia es larga, el texto de más se desborda hacia la línea de **arriba** de su propio renglón, y `pending_prereq_groups` ya lo captura antes de llegar al código (patrón "antes"). Pero a veces el desborde cae hacia la línea de **abajo** — en el renglón visual de la materia SIGUIENTE, o incluso partiendo una clave a la mitad (`"CON-"` en una fila, `"10001"` en la que sigue). Sin manejo especial, ese texto se pierde en silencio (si hay un stub abierto) o se pega por accidente a la materia que se encuentre después (patrón "antes" aplicado a la materia equivocada) — así es como `COM-22108` quedaba con `prerreqs: []` mientras `COM-23114` heredaba un prerreq ajeno de más, en cascada.

No hay forma de saber, con solo mirar la fila, si un bloque de prerreqs "huérfano" (sin código propio en esa fila) pertenece hacia atrás (desborde de la materia ya guardada) o hacia adelante (prerreqs propios de la materia que sigue, patrón normal) — a veces incluso **ambas** cosas pasan en el mismo hueco entre dos materias (cada una desborda un poco hacia el mismo espacio). Se resuelve por geometría, no por contenido: al encontrar la siguiente materia, se arma la secuencia de tops `[último guardado directo, cada fila acumulada, materia nueva]` y se busca el hueco vertical más grande entre consecutivos — todo lo que quede **antes** de ese hueco se atribuye hacia atrás (se fusiona en los prerreqs de `last_saved_course_id`); todo lo que quede **después** se dispara tal cual, como prerreqs propios de la materia nueva. Un desborde simple (un solo bloque, mucho más cerca de la materia de arriba que de la de abajo) cae completo de un lado; dos desbordes consecutivos hacia el mismo hueco quedan separados justo por el hueco más grande entre ambos bloques.

Este mecanismo solo aplica cuando `last_saved_course_id` sigue vigente (la materia inmediatamente anterior se guardó **directo**, sin pasar por stub) — se limpia en cualquier cambio de contexto (nueva materia encontrada, encabezado de área/semestre, o cuando una fila de solo-nombre indica que ya empezó a describirse la materia siguiente). Si no hay `last_saved_course_id`, el bloque se usa completo como prerreqs de la materia nueva (comportamiento original, sin ambigüedad que resolver).
...
**Slots de optativa dentro de la tabla de un semestre** (`OPTATIVA_SLOT_RE`, ej. "Optativa", "Optativa I", "Optativa de Estadística", "Optativa Área de Concentración" — sin clave real, distinto del catálogo de arriba): se capturan como `(current_semester, creditos)` en `optativa_credits` (créditos con fallback a 6 si no se pudo leer de esa fila) y al final de `parse_pdf` se agregan como materias sintéticas `OPTATIVA-1`, `OPTATIVA-2`, ... (`nombre: "Optativa I"`, `"Optativa II"`, ... vía `to_roman`), sin `prerreqs`, cada una en su **semestre real** (el que traía su fila, no `max + 1`) — quedan mezcladas con las materias reales de ese semestre, no en una columna final aparte (mismo patrón que "Materia N de Área de Concentración" abajo; antes de este fix sí caían todas juntas en `max_semestre + 1`). La numeración (`OPTATIVA-1`, `OPTATIVA-2`, ...) sigue siendo global y secuencial por orden de aparición en el PDF, sin reiniciar por semestre. El regex reconoce **cualquier** variante que empiece con "Optativa"/"Optativas" (antes solo reconocía la forma corta tipo "Optativa I"; las variantes con nombre se colaban al nombre de la siguiente materia real — bug ya corregido).

**Slots de "Materia N de Área de Concentración"** (`AREA_CONCENTRACION_SLOT_RE`): mismo mecanismo de captura que optativas (mismos dos puntos de detección en `parse_pdf`, mismo patrón de preservar `current_semester` en el momento de la detección) — representan una materia obligatoria de esa etapa del plan, no una optativa libre. A diferencia de optativas, se preserva el número que ya trae el PDF (`AREA-{n}`, `nombre: "Materia {n} de Área de Concentración"`) en vez de renumerar — es estable y sin ambigüedad, porque cada slot de área sí trae un número propio en el PDF (las optativas no). `ECO-I.pdf` usa "Electiva N de área de concentración" en vez de "Materia N..." para el mismo tipo de slot — el regex acepta ambos prefijos, el nombre sintético de salida sigue siendo siempre "Materia N de Área de Concentración" (se genera del número capturado, no del texto que matcheó). Dos de las 5 áreas de `ECO-I` (Finanzas, Empresarial) traen un único slot de este tipo **sin numerar** ("Electiva de área de concentración", sin "N" — el PDF no numera cuando solo hay uno); `AREA_CONCENTRACION_SLOT_SINGULAR_RE` cubre ese caso como fallback, tratándolo como slot `1` (seguro porque `area_concentracion` no se hereda entre secciones — nunca hay conflicto con un "Electiva 1..." explícito de otra área).
