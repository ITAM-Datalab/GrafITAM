...
Botón "¿No encuentras tu materia, grupo o plan?" que abre un modal (tipo de problema, clave de materia, nombre, grupo/CRN, carrera/plan, comentario). Dos caminos de envío, ambos arman una URL prellenada y la abren en pestaña nueva (cero backend propio; no se pueden adjuntar PDFs vía querystring, solo describir el plan/carrera en texto):

- **"Enviar reporte" (primario, `handleSubmitGoogleForm`)**: arma un link de Google Forms prellenado (`docs.google.com/forms/d/e/{GOOGLE_FORM_ID}/viewform?usp=pp_url&entry.NNN=...`) al form público "Reportar Problema - GrafItam" (`https://forms.gle/kG7GuWgdZoAYrye3A`, no requiere cuenta de Google) — pensado para estudiantes sin cuenta de GitHub, que es la mayoría. `GOOGLE_FORM_ENTRIES` mapea cada campo del modal a su `entry.NNNNNNNN` (sacados del HTML público del form, variable `FB_PUBLIC_LOAD_DATA_`, no de la API — **si Braulio agrega/quita/reordena una pregunta del form hay que volver a sacarlos**, ver `TIPO_FORM_OPTION` para el caso especial del campo "Tipo de problema": 3 valores mapean a una opción de texto fijo del form, `otro` usa el mecanismo `__other_option__`/`other_option_response` de Google Forms). No son datos sensibles — viven hardcodeados en el componente, sin env var ni secret de CI.
- **"o repórtalo en GitHub" (secundario, discreto, `handleSubmitGithub`)**: comportamiento original sin cambios — arma `github.com/ITAM-Datalab/GrafITAM/issues/new?title=...&body=...` y la abre en pestaña nueva; el usuario le da clic a "Submit" del lado de GitHub. Se deja para quien sí tenga cuenta y prefiera ese flujo.
