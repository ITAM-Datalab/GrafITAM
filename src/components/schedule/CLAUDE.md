...
Grid semanal (LU-SA, horas 7:00-22:00, 1px = 1 minuto) de los grupos en `selectedGroups`, con fondo `bg-base-cream` y líneas de hora/día marcadas (`itam-dark/25`) para que se vean sobre el fondo de la app. El grid tiene un `min-width: 560px` y su contenedor es `overflow-x-auto` — en pantallas angostas el fallback es scroll horizontal en vez de columnas de día ilegibles (las 6 columnas usan `1fr`, sin piso propio). Cada bloque usa el color de su materia (`coursePalette.ts`); si hay traslape con otro bloque seleccionado, se le agrega un borde punteado `#8C5E58` encima (no se repinta de rojo sólido, para no perder de vista de qué materia es). El bloque muestra 3 líneas: clave + nombre, salón, y profesor(es) + horario — con wrap normal (no `truncate` de una sola línea) y una altura mínima de 26px para que hasta las clases más cortas alcancen a mostrarlas; el `overflow-hidden` del bloque sigue como red de seguridad si aun así no caben, y el `title` del div trae el detalle completo (incluyendo salón) para hover.

`groups` (prop, filas seleccionadas de potencialmente varias materias) se agrupa primero por `courseId+crn` y luego por sesión vía `groupSessions` (`src/algorithms/scheduleOverlap.ts`) — un CRN con varios profesores en el mismo horario/día (team-teaching, ej. `ADM-12302`) genera **un solo bloque** con los profesores unidos por `" / "`, no uno por fila; teoría y laboratorio (horario/día distintos) se siguen dibujando como bloques separados.
...
Botón "Reporta un problema" que abre un modal (tipo de problema, clave de materia, nombre, grupo/CRN, carrera/plan, comentario). Sin backend propio (GitHub Pages) — el envío primario le pega directo a un webhook de Make.com, con fallback a un Google Form si ese webhook falla:

- **"Enviar reporte" (primario, `handleSubmit`)**: `POST` a `import.meta.env.VITE_MAKE_WEBHOOK_URL` con `{ title, body, tipo, clave }` (`title`/`body` armados por `buildIssueContent`, mismo formato Markdown de siempre) y header `x-make-apikey: {VITE_MAKE_WEBHOOK_API_KEY}`. Make valida esa API key de forma nativa en el propio webhook (sección "API Key authentication" al crearlo/editarlo, configurable con una o más keys) **antes** de correr el escenario — una request sin key válida ni siquiera dispara el módulo de GitHub, no hace falta un Filter manual comparando un secreto en el body. El escenario en sí es solo Webhook → módulo GitHub "Create an Issue", que crea el issue en `ITAM-Datalab/GrafITAM` sin que el estudiante necesite cuenta de GitHub ni cambiar de pestaña. La conexión de GitHub en Make usa un **Personal Access Token fine-grained** (no OAuth — OAuth de GitHub no permite acotar el acceso a un solo repo desde la pantalla de autorización, solo por scopes globales) con **Resource owner: `ITAM-Datalab`**, **Repository access: Only select repositories → `GrafITAM`**, permiso **Issues: Read and write** — así el token de la cuenta de Make (compartida con el datalab) no puede tocar otros repos de la organización. La API key sigue viviendo en el bundle JS del cliente igual (100% client-side, sin backend) — filtra bots/scrapers casuales, no es seguridad real contra alguien que inspeccione el bundle.
- **Fallback a Google Form (`handleSubmitGoogleForm`)**: si el fetch al webhook falla (`submitState === 'error'`), aparece un botón "Reportar por Google Form" que arma un link prellenado (`docs.google.com/forms/d/e/{GOOGLE_FORM_ID}/viewform?usp=pp_url&entry.NNN=...`) al form público "Reportar Problema - GrafItam" (`https://forms.gle/kG7GuWgdZoAYrye3A`, no requiere cuenta de Google) y lo abre en pestaña nueva. `GOOGLE_FORM_ENTRIES` mapea cada campo del modal a su `entry.NNNNNNNN` (sacados del HTML público del form, variable `FB_PUBLIC_LOAD_DATA_`, no de la API — **si Braulio agrega/quita/reordena una pregunta del form hay que volver a sacarlos**, ver `TIPO_FORM_OPTION` para el caso especial del campo "Tipo de problema": de las 6 categorías, 3 mapean a una opción de texto fijo del form (`materia_faltante`, `grupo_incorrecto`, `plan_faltante`) y 3 usan el mecanismo `__other_option__`/`other_option_response` de Google Forms (`otro`, y las 2 categorías nuevas `horario_incompleto`/`plan_dato_incorrecto`, que todavía no existen como opción fija en el form real)). No son datos sensibles — viven hardcodeados en el componente, sin env var ni secret de CI.
- **`TipoProblema`** (6 categorías, catálogo único sin filtrar por pestaña activa — el componente no recibe props ni lee ningún store, así que el `<select>` es el mismo sin importar desde qué pestaña se abrió): `materia_faltante`, `grupo_incorrecto`, `horario_incompleto` (falta una sesión dentro de un grupo que sí existe, ej. laboratorio), `plan_faltante`, `plan_dato_incorrecto` (dato mal parseado del plan/DAG: créditos, prerrequisitos, número de optativas, área), `otro`. Qué campo es obligatorio por tipo vive en `REQUIRED_FIELD` (una sola tabla, reusada por `isValid` y los 2 labels condicionales del form).
- **Env vars** (`VITE_MAKE_WEBHOOK_URL`, `VITE_MAKE_WEBHOOK_API_KEY`, ver `.env.example`): en CI, `deploy.yml` las inyecta desde los GitHub Actions secrets `MAKE_WEBHOOK_URL`/`MAKE_WEBHOOK_API_KEY` al correr `npm run build`. Quedan igual visibles en el bundle final compilado (no hay forma de ocultarlas sin backend) — el propósito de usar env vars es no dejarlas en texto plano en el historial de git y poder rotarlas sin commit.
