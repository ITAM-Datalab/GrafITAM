...
- **Sin límite de resultados**: se muestran todos los matches, no solo los primeros N — la `<ul>` de resultados ya es `max-h-80 overflow-y-auto` (scrollable), así que truncar no ganaba nada en usabilidad y sí escondía resultados válidos.
- **Orden de resultados por código de plan**: como `allPlanMetas` (`src/data/loader.ts`) ya viene ordenado por `programa → letra → área`, filtrar directo sobre él preserva ese orden — sin round-robin ni diversificación por programa (hubo una versión anterior con ambos que se quitó porque escondía variantes del mismo programa entre sí, ej. `CDA-B`/`CDA-C` quedaban relegados detrás de programas con una sola variante al buscar "Datos"). Con un solo grupo, el resultado es simplemente el filtro en orden de código. Con más de un grupo (query compuesta, ej. "Economía y Relaciones Internacionales"), los planes que matchean **todos** los grupos van primero (ordenados por código); después los que matchean solo parte, también ordenados por código.
